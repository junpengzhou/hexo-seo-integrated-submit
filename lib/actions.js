module.exports = function () {
  ...
  if (!config) {
    console.warn('Sorry, hexo-seo-integrated-submit Config is null, please config the hexo-seo-integrated-submit, get the manual in https://github.com/junpengzhou/hexo-seo-integrated-submit!')
    return;
  }
  ...
  const google_run_shell = 'npx hsis-google "${{secrets.google_client_email}}" "${{secrets.google_private_key}}"'
  const bing_run_shell = 'npx hsis-bing ${{secrets.bing_apikey}}'
  const baidu_run_shell = 'npx hsis-bing ${{secrets.baidu_token}}'
